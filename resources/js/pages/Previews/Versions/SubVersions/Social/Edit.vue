...
const breadcrumbs = [
    ...
    { title: 'Edit Social Sub Version', href: '#' },
];
...
const handleSubmit = () => {
    ...
    axios.post(`/previews/version/social/edit/subVersion/${subVersion.value.id}`, payload)
        .then((response) => {
            Swal.fire('Success', 'Social SubVersion updated successfully.', 'success');
        })
        .catch(() => {
...
<template>
    ...
    <Head title="Edit Social Sub Version" />
        ...
        <div class="p-6 max-w-4xl mx-auto space-y-6 w-full">
            ...
            <div>
                ...
                <input v-model="form.name" type="text"
                    ...
                    placeholder="e.g. Social SubVersion 1" required />
            </div>
            ...
            <!-- Social List (only if new images are uploaded) -->
            <draggable v-if="form.socials.length > 0" v-model="form.socials" item-key="file.name" handle=".handle"
