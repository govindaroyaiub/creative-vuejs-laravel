                    ...
                    <tbody>
                        <tr v-for="(log, index) in logs.data" :key="log.id">
                            ...
                            <td class="py-2 px-4 border-b text-center">
                                <span v-if="log.causer">{{ log.causer.name || log.causer.email || log.causer.id
                                    }}</span>
                                <span v-else>-</span>
                            </td>
                            <td class="py-2 px-4 border-b text-center">
                                    ...
                                    <div v-if="log.properties && log.properties.attributes">
                                        ...
                                        <ul class="text-xs">
                                            <li v-for="(value, key) in log.properties.attributes" :key="key">
                                                ...
                                                <span
                                                    v-if="log.properties.old && log.properties.old[key] !== undefined">
                                                    (was <span class="text-red-600">{{ log.properties.old[key]
                                                        }}</span>)
                                                </span>
                                            </li>
                                        </ul>
